...
**Full completion** *(Leksokipos)* — Local end-state reached when 0 Valid Words remain (player has found every word). "ΤΟ ΠΕΘΑΝΕΣ" is displayed in the word-feedback area and the board locks (letter input disabled). Purely a game mechanic — no reward. *(Formerly **Τζιμάνι**: a secret rank + one-shot achievement + 🏛️ Leaderboard glyph. The whole perfect-round / Τζιμάνι concept was **removed 2026-07-18** across achievement, stat cell, glyph, and score wire — ADR 0013 amendment. The `leksokipos-tzimani` id is retired and its earned rows deleted, freeing it for a possible future re-award under different conditions.)*
...
**Words by Length** (Λέξεις ανά μήκος) — A Profile Page card showing how many long words the player has ever found, broken down by word length: lengths 10, 11, 12 individually plus a "13+" tail, each a small horizontal bar, over a total. Only words of 10+ letters are tracked at all — a word that long is itself a one-shot Achievement (Σιδηρόδρομος = 10, plus the 11/12/13 badges), and `/api/words` drops anything shorter (the floor is `WORDS_MIN_TRACKED`, derived from `achievementTuning.wordLengthBadges`). Fed by the append-only `player_words` set (one row per valid ≥10 find per `puzzle_date`), aggregated in Postgres by the `player_words_by_length` RPC — the read never fetches rows (`GET /api/profile/words` → `bucketWordsByLength`). Leksokipos-only in v1 (the only Game with free-form found words); a cross-Game extension is a `game_id` column value, not a rewrite. **No backfill exists** — finds before capture shipped were never stored — so everyone starts at zero and the empty state never implies lost history. Display-only (no Badge; those are parked). Rides `FEATURE_FLAGS.achievements` like the pangram/achievement writes, so it is dark until launch. (Not: word count, vocabulary stats)

**Lifetime Stats** — Per-player aggregates over full `game_scores` history (append-forever makes them safe): total points, puzzles played, pangram count (from the separate `player_pangrams` set, not `game_scores`), and the Podium Counts (🥇🥈🥉 in one Βάθρο strip cell — Leksokipos only in v1). *(A Τζιμάνι count cell was removed 2026-07-18 with the perfect-round concept — ADR 0013 amendment. The former single Πρωτιές cell became the three-medal Βάθρο cell the same day.)* Streak is defined below but not yet surfaced in the strip. Keyed by DeviceId — never `auth_user_id` (Sign-in Restore makes the adopted DeviceId canonical, so one key serves anonymous and AuthLinked players alike); Daily Puzzles only (Custom Puzzles never post scores). (Not: statistics, records — records are all-time bests, a parked pillar)

**Podium Finish** — Placing 1st, 2nd, or 3rd on a Game's Daily Puzzle Leaderboard for one `puzzle_date`, under **competition ranking**: a player's rank that day is `1 + (players with a strictly higher score)`. Ties share a rank, and a shared rank consumes the ranks below it — `90, 90, 80` puts both 90s at rank 1 and the 80 at rank 3, so that day has **no** 2nd. This matches the Leaderboard's count-of-better-scores rank formula, so Profile and Leaderboard agree. Since every Leaderboard is higher-is-better (ADR 0014), rank 1 is the day's maximum `score`. **First-Place Finish** is the rank-1 tier of this podium. (Not: win, top three by any other tie rule)

**Podium Counts** — The lifetime tallies of a player's Podium Finishes: First-Place Count (**Πρωτιές**), Second-Place Count (Δευτεριές), Third-Place Count (Τριτιές). Leksokipos-only in v1; extends per-Game later (each Game contributes its own daily podium). **Derived** from append-forever `game_scores` (data-class 2), never stored — computed in one pass by `countPodiumFinishes` (`src/lib/placement.ts`) over all of a Game's rows, index-backed by `game_scores_game_date_score_idx`; `countFirstPlaceFinishes` is the rank-1 wrapper kept for the stable `leksokipos_first_place_count` field. The strip shows all three in a single 🥇🥈🥉 Βάθρο cell (three cells with spelled-out labels would overflow `max-w-game`). Tiered Podium Badges are a deferred follow-up (frozen `leksokipos-first-place-*` ids chosen when built). (Not: wins count)
...
**Offline Mode** — A deliberate, platform-wide client-side state, toggled from the Shell drawer, that lets a player keep playing the finished games with no network. On activation — **while the player is still online**, the load-bearing assumption — it prefetches the offline games' routes, then blocks browser refresh (`beforeunload`) and confirms before any navigation that leaves the prefetched set. Warm start only: every game page is `force-dynamic`, so a page not already in memory cannot load without the server, which is why a refresh is blocked rather than merely warned about. Released manually; deactivation flushes the Offline Score Outbox. The offline set is derived from `GAME_REGISTRY.wip`, minus the community surfaces (`stavrolekso`, `leksikastirio`) whose content is fetched per view. *(Renamed from **Offline Lock**, which named the superseded Leksokipos-only toggle — ADR 0010, amended 2026-08-03.)* (Not: airplane mode, PWA, service worker, cold start)

**Offline Score Outbox** — A localStorage record (`wordgames:offline-outbox`) holding pending Scores earned during Offline Mode, **keyed by `(gameId, puzzleDate)`** and overwriting per key: `{ gameId, puzzleDate, deviceId, score, displayName }`. Not an append queue — `game_scores` upserts by `(device_id, game_id, puzzle_date)`, so only the latest Score per key matters; keying per game stops a second game played offline from discarding the first game's pending Score. Flushed to `game_scores` on deactivate, or on the next page mount if an entry exists (the safety net for a player who forgot to deactivate). Kept on flush failure and retried. Flushing bypasses `useScoreSubmission` and calls `postScoreAwaitable` directly, since `postScore` is fire-and-forget and cannot report failure. The **one documented exception** to "`useGameStore` is the only localStorage writer" — it is not game state, so it gets its own key. **Only Leksokipos queues today**; the other games are playable offline but their Scores are lost (issue 15). (Not: queue, cache, retry buffer)
...
| `player_words` | Append-only word find-set feeding the Words by Length card (`20260718120000`): one row = one valid `word` a device found on one `puzzle_date`, with a server-stamped `length` and a `game_id` (default `leksokipos`, left out of the UNIQUE until a second Game needs it). `UNIQUE(device_uuid, puzzle_date, word)`, insert-if-absent. **Only 10+ letter finds are stored** — `/api/words` drops anything below `WORDS_MIN_TRACKED`, and migration `20260726120000` purged the historical short rows — which keeps this (the fastest-growing table) small. Sibling of `player_pangrams`, not a subset. Per-length counts read via the invoker-rights `player_words_by_length` aggregate RPC (never fetches rows). Anon RLS = SELECT+INSERT only, append-forever — never swept. Unioned on Sign-in Restore via `planWordsMerge`. Beta capture data (ADR 0013 launch-reset class) gated behind `FEATURE_FLAGS.achievements`. |
