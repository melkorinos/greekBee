...
**Slot-Fill Family** — The Worldle-shaped games: **Topothesies**, **Πόσο κάνει;**, **Λογοπαίγνιο**, and **Λεξόπλεγμα** (joined 2026-07-27). The player fills a slot (a Regional unit, a price, a brand, a traced word) from a visual prompt; every stage flag is *derived* from the guess history; giving up is an explicit end-state; the Score decays with each wrong guess and is posted continuously while the round is live. Spine: `useSlotFillRound`. Λεξόπλεγμα's own variation lives in its reducer, not in the spine — its `RESTORE_STATE` filters restored words against the current puzzle, which no other member does. Λεξοδρομία is **not** a member and must never be migrated (it owns a decay clock and a restore that must interleave with it) — see ADR 0019. (Not: worldle games, daily-photo games)
...
**Round End** — The moment a Session becomes summarisable, and the trigger for the Result Panel. Deliberately **not one rule**: each Game names its own and the list is closed — Λεξιαρχείο when all five Lengths are **resolved** (won *or* lost, never "all won"); Βρες τη Φράση when its status leaves playing; Λεξοδρομία when all ten words are resolved; Λεξόπλεγμα when every Required Word is found; Τοποθεσίες when its stage is finished, give-up included; **Λεξόκηπος when the top Rank is reached on a Daily Puzzle** — the one Game with no terminal state, where play continues past its own Round End. A lost round reaches Round End exactly like a won one. See ADR 0025. (Not: game over, finish, completion — Full completion is a different Leksokipos concept)

**Result Panel** — What Round End renders: the Score, the Game's own one-line summary, the share action, and the Leaderboard link. Inline below the board for every Game except **Λεξόκηπος**, which pops once per Daily Puzzle because play has not stopped. Λεξοδρομία's and Λεξόπλεγμα's per-word recaps sit inside it. The text it shares is spoiler-free by construction and carries a link to the Game — never to a specific date, so a message resurfacing weeks later still sends the reader to today. Deliberately **not** a `GameCapability`: it writes nothing to the shared database, so by ADR 0020's criterion it is presentation. See ADR 0025. (Not: end screen, results modal, share card — that is the Open Graph image)
...
**Miss Rule** — What a Game serves on a date its own content does not cover. Each Game selects its Daily Puzzle differently (hand-authored calendar, Community Puzzle first, collision-skipping rotation) and that variation is deliberate, but the miss rule is **one platform-wide invariant**, not a per-Game choice: a miss (1) always answers — no throw, no empty page; (2) never freezes on one board, so serving "the last row" forever is forbidden; (3) never runs ahead of the requested date, so a row pinned to a future day cannot surface early. `pickByDateOrRotate` in `src/lib/puzzleRotation.ts` implements it for the two hand-authored calendars (Leksokipos, Πόσο κάνει;) — exact date match, else `dateToIndex` rotation over the rows already **due** — and `src/test/shared/dailyPuzzleSelection.test.ts` asserts all three parts against every Game. Only Leksokipos can actually exhaust its content, so that same file carries a 180-day horizon check that fails well before the calendar ends. (Not: fallback puzzle, default puzzle)
...
**Phrase Pool** *(Vres Tin Frasi)* — Source of daily Phrases: static JSON of ~500 pre-computed Greek phrases (`phrases-el.json`), rotated deterministically by date. The Game accepted community phrases until 2026-08-20, when submission was removed with the queue still empty (ADR 0027).
...
**Badge** — The visual token of an Achievement: a full tile in the Trophy Case, or the player's selected Display Badge on Leaderboard rows. Drawn as a **`BadgeMark`** since 2026-08-10 (ADR 0013 §7) — a tier-coloured ring, a soft disc, and a `currentColor` mark, at three sizes (chip 14px / tile 32px / toast 34px). **A tier is a colour, never a different drawing**, so five marks cover all five Achievements across four tiers. Emoji `glyph`s, `TIER_MEDALS` (🥉🥈🥇), the 🔒 and the `grayscale` locked treatment are all **deleted**, not dormant. (Not: achievement — that's the earnable condition; not: glyph, emoji)

**Display Badge** — The ONE earned Achievement a player opts to show beside their name on every Game's Leaderboard: a distinct chip after the plain name, carrying the `BadgeMark` in the **highest earned tier's** colour, resolved at read time so it auto-upgrades. Picked (and cleared, by tapping again) in the Trophy Case; stored as the **base** achievement id in `player_profiles.selected_badge_id` (never a tier id); `POST /api/profile/badge` verifies the Achievement is genuinely earned. A retired id resolves to null rather than crashing. Default is none — opt-in. **Exactly one, permanently** — displaying several Badges with precedence rules is a closed question, not a deferred one (ADR 0013 amendment 2026-08-06). (ADR 0013 amendment 2026-07-18; art per §7.) (Not: title, flair, part of the DisplayName)
...
**Trophy Case** — The full Achievement display on the Profile Page: every catalog entry rendered, earned Badges lit in their tier colour, locked ones in a **neutral frame with the mark still visible** and their unlock hint (never dimmed or greyed — a locked Badge must still show what it is; ADR 0013 §7). Sits with Words by Length inside one labelled **Leksokipos section** — structural scoping, so "Badges are Leksokipos-only" needs no caveat line. **Not tabs**: tabs advertise a sibling to switch to, and exactly one Game earns Badges; a second earning Game is what would justify them. (Not: badge list, achievements tab)
...
**Edit PIN** — Alphanumeric code (4–8 chars) carried by a Stavrolekso Puzzle from submission. Stored plain on the row. Combined with the puzzle's unique ID to authenticate edit access. Only valid while the puzzle is `pending`. ADR 0005 decided this as *creator-chosen*; the Maker in fact **generates** one and shows it, never offering a field to pick it — the wire still accepts either (ADR 0005 amendment 2026-08-14). (Not: edit token, edit code)

**Maker** — The authoring surface for a Community Puzzle, as opposed to the player surface. Only Stavrolekso has one (`/stavrolekso/maker`). Three steps: pick a grid size, shape the grid by toggling black squares, then fill in letters and clues. It is not a Game — it has no Session, no score and no Puzzle of the day, and its state never enters `useGameStore`. Its editing rules are pure (`games/stavrolekso/lib/makerReducer.ts`), so what a click or keystroke does to the grid is decided outside React. (Not: editor, builder, constructor)

**Run** — The cells one Slot occupies, in reading order. The unit the cursor walks when typing in the Maker, and the boundary that stops a backspace from reaching into the Slot before it. (Not: line, span)
...
**Cue** — A named sound-worthy moment, and the short sound it plays. Named for **the moment, not the noise** (`pangram`, `wordFound`, `missingCenter`), so replacing the rooster is a file swap rather than a rename. Three are registered, all Leksokipos, all chosen by one pure `selectSoundCue` over the submission's `ValidationResult` — but since 2026-08-25 **only `wordFound` is audible**: a Pangram plays that same blip rather than the rooster, and every rejection is silent. The other two are silenced **at the selector, not deleted** — rows and MP3s still ship, so restoring either is one line. **ON by default since 2026-08-26, and there is no player-facing control** — the Shell header's 🔊/🔇 toggle was removed that day, and a default of off would have made the one surviving Cue unreachable by anybody. The preference still lives standalone under `localStorage["sound-preference"]`, outside the state envelope, and a stored `"off"` is still honoured; what silences the Platform now is `FEATURE_FLAGS.soundCues`, which gates `useSoundCue.play` and is the only off switch there is. Deliberately **not** a `GameCapability` (ADR 0020's criterion is what a Game does to the shared database, and a Cue writes nothing). Registry, per-Cue volume and file provenance live in `src/config/sound.ts`; the licence bar is CC0 or the Pixabay Content License, never CC-BY. See ADR 0021. (Not: sound effect, notification, chime)

**Backup** — Producing the artifact: `npm run db:backup` dumps roles, schema and data, then packs them into an AES-256 encrypted `.7z`. **The archive is the artifact; the folder beside it is scaffolding.** There is deliberately no unencrypted fallback — `pg_dump` carries `auth.users`, so an unprotected dump is every signed-in player's email address sitting on a disk. A Backup that has not left the machine is a copy, not a Backup. (Not: Restore, Rehearsal, snapshot, export)

**Restore** — Recovering a database that is gone or corrupted, by replaying a Backup into a **fresh Supabase project**: roles, then schema, then data, in that order. The emergency path, run at most once, and the reason the free organization's second project slot is held empty rather than spent on staging (ADR 0024). Distinct from `admin-restore.md`, which recovers **one player's identity** while the database is alive. (Not: Backup, Rehearsal, rollback)

**Rehearsal** — Replaying a Backup into a **local** PostgreSQL database and applying pending migrations against those real rows, to find out what a migration does before `db push` does it for real. Routine, repeatable, and never touches the hosted project. What it catches is **data-shape failure** — a `NOT NULL` meeting existing nulls, a unique index meeting existing duplicates — which is exactly the class an empty staging database passes. A pass means *the migration applied*, not *the suite is green*: the live-DB tests go through Supabase's HTTP API, which a local PostgreSQL install does not have. Run it with `npm run db:rehearse` (`scripts/rehearse-migration.ps1`); see ADR 0024 and `docs/disaster-recovery.md`. (Not: Restore, Backup, staging, dry run)
...
| `game_scores` | Unified leaderboard for all games, keyed by `game_id` + `device_id`. Which ids may appear is **derived, not listed** — it is the set of registry rows declaring the `scores` capability (`gameIdsWith("scores")`, ADR 0020), Πόσο κάνει; and Λογοπαίγνιο declare no capabilities while they run on placeholder content, so they write nothing; every other registered Game posts here. Λεξιαρχείο and Βρες τη Φράση lost theirs before launch (ADR 0027) and got them back on 2026-08-30 (ADR 0028) — their beta rows were never deleted, so both boards resumed on top of real history. **`data` jsonb** holds Λεξιαρχείο's per-length fold and nothing else — its only writer. It was dropped 2026-08-21 with that Game's scoring (migration `20260820120000`, ADR 0027 §5) and re-added 2026-08-30 with `not null default '{}'` when the scoring came back (migration `20260830120000`, ADR 0028); the 41 rows written before the drop keep their `score` totals and carry an empty object where their old breakdown was. `is_perfect` went in the 2026-08-21 migration too (ADR 0013's amendment) and stays gone. The route accepted no client-supplied `data` at all (Leksokipos posted unread `{ words, pangrams }` there until 2026-08-16). Device-keyed only — no `auth_user_id` column; Sign-in Restore makes the adopted DeviceId canonical, so device_id serves anonymous and AuthLinked players alike (the device→account map lives in `player_profiles`). |
...
Rows are never pruned. The 7-day leaderboard window is query-side only. Lifetime Stats, Streaks, and the derived-on-read lifetime-point Achievements all read full `game_scores` history, so deletion would silently corrupt them. (Achievements themselves are not backfilled — they start at zero at launch — but their live derivation from post-launch history still depends on nothing being pruned.) When the 50 000-row tripwire is crossed, the answer is "raise the tripwire / optimize storage" — never "prune history." **Until 2026-07-05 this was policy only — the daily `/api/cleanup-scores` cron still deleted `game_scores` older than 10 days (issue 03), so "Lifetime" Stats were really last-10-days stats.** The cron now prunes only the ephemeral tables (`game_state`, `transfer_codes`) governed by `SESSION_RETENTION_DAYS`; `game_scores` is untouched.

**One deliberate exception exists and is not a precedent:** `supabase/scripts/launch-reset.sql`, the pre-launch wipe (ADR 0012's 2026-08-07 amendment). It is committed, reviewable, and **never auto-applied** — the operator runs it by hand on release day, after the launch deploy. It empties `game_scores`, `game_state`, `player_achievements` and `player_milestones` and NULLs `player_profiles.selected_badge_id`, while **keeping** every `player_profiles` row and all community content (`nominations`, `nomination_votes`, every `community_*_puzzles` table, `identity_audit`, `transfer_codes`) — that content was earned by review work, not by playing. It is also what licenses the three frozen-achievement-id exceptions. After it runs, append-forever resumes with no further exceptions.
