...
| `game_scores` | Unified leaderboard for all games, keyed by `game_id` + `device_id`. Which ids may appear is **derived, not listed** — it is the set of registry rows declaring the `scores` capability (`gameIdsWith("scores")`, ADR 0020), today `leksokipos`/`leksindeseis`/`leksodromia`/`leksoplegma`/`topothesies`. Πόσο κάνει; and Λογοπαίγνιο declare no capabilities while they run on placeholder content, so they write nothing; Λεξιαρχείο and Βρες τη Φράση lost theirs outright before launch (ADR 0027). **There is no `data` jsonb** — dropped 2026-08-20 (migration `20260820120000`, ADR 0027 §5) once Λεξιαρχείο's per-length fold, its only writer, went with that Game's scoring. `is_perfect` went in the same migration (ADR 0013's amendment). The route accepted no client-supplied `data` at all (Leksokipos posted unread `{ words, pangrams }` there until 2026-08-16). Device-keyed only — no `auth_user_id` column; Sign-in Restore makes the adopted DeviceId canonical, so device_id serves anonymous and AuthLinked players alike (the device→account map lives in `player_profiles`). |
...
**"Score" is overloaded** — each scoring Game defines its own Score term above, and they are not the same currency: Leksokipos Score = accumulated word points, Leksoplegma = word length × 10 plus a bonus, Topothesies = shape points + capital bonus. All are higher-is-better (ADR 0014) and all travel as the API field `score`, for interface compatibility only.
