...
| `game_scores` | Unified leaderboard for all games, keyed by `game_id` + `device_id`. Which ids may appear is **derived, not listed** — it is the set of registry rows declaring the `scores` capability (`gameIdsWith("scores")`, ADR 0020), today `leksokipos`/`leksiarxeio`/`leksindeseis`/`vrestifrasi`/`leksodromia`/`leksoplegma`/`topothesies`. Πόσο κάνει; and Λογοπαίγνιο declare no capabilities while they run on placeholder content, so they write nothing. Leksiarxeio writes one row per `word_length`. **The `data` jsonb is Leksiarxeio's alone** — `{"4":…,"8":…}`, per-length points folded server-side by `mergeLengthScore`; every other Game leaves it `{}`, and the route accepts no client-supplied `data` at all (Leksokipos posted unread `{ words, pangrams }` there until 2026-08-16). Device-keyed only — no `auth_user_id` column; Sign-in Restore makes the adopted DeviceId canonical, so device_id serves anonymous and AuthLinked players alike (the device→account map lives in `player_profiles`). |
