...
**Badge** — The visual token of an Achievement: a full tile in the Trophy Case, or a single inline glyph on Leaderboard rows. (No inline Leaderboard glyph is live today — the 🏛️ Τζιμάνι precedent was retired 2026-07-18; see Full completion.) (Not: achievement — that's the earnable condition)
...
**Words by Length** (Λέξεις ανά μήκος) — A Profile Page card showing how many valid words the player has ever found, broken down by word length: lengths 4…9 individually plus a "10+" tail, each a small horizontal bar, over a total. Fed by the append-only `player_words` set (one row per valid find per `puzzle_date`), aggregated in Postgres by the `player_words_by_length` RPC — the row count grows far faster than any other fact table, so the read never fetches rows (`GET /api/profile/words` → `bucketWordsByLength`). Leksokipos-only in v1 (the only Game with free-form found words); a cross-Game extension is a `game_id` column value, not a rewrite. **No backfill exists** — finds before capture shipped were never stored — so everyone starts at zero and the empty state never implies lost history. Display-only (no Badge; those are parked). Rides `FEATURE_FLAGS.achievements` like the pangram/achievement writes, so it is dark until launch. (Not: word count, vocabulary stats)
