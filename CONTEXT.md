...
**Podium Finish** — ⚠️ *Retired 2026-08-06 (ADR 0013 amendment): tiered Podium Badges were rejected and the whole podium lane is decided-for-deletion. The term still describes live code until the removal slice lands; delete this entry with it.* — Placing 1st, 2nd, or 3rd on a Game's Daily Puzzle Leaderboard for one `puzzle_date`, under **competition ranking**: a player's rank that day is `1 + (players with a strictly higher score)`. Ties share a rank, and a shared rank consumes the ranks below it — `90, 90, 80` puts both 90s at rank 1 and the 80 at rank 3, so that day has **no** 2nd. This matches the Leaderboard's count-of-better-scores rank formula, so Profile and Leaderboard agree. Since every Leaderboard is higher-is-better (ADR 0014), rank 1 is the day's maximum `score`. **First-Place Finish** is the rank-1 tier of this podium. (Not: win, top three by any other tie rule)

**Podium Counts** — ⚠️ *Retired 2026-08-06 with Podium Finish above — delete both when the removal slice lands.* — The lifetime tallies of a player's Podium Finishes: First-Place Count (**Πρωτιές**), Second-Place Count (Δευτεριές), Third-Place Count (Τριτιές). Leksokipos-only in v1; extends per-Game later (each Game contributes its own daily podium). **Derived** from append-forever `game_scores` (data-class 2), never stored — computed in one pass by `countPodiumFinishes` (`src/lib/placement.ts`) over all of a Game's rows, index-backed by `game_scores_game_date_score_idx`; `countFirstPlaceFinishes` is the rank-1 wrapper kept for the stable `leksokipos_first_place_count` field. The strip shows all three in a single 🥇🥈🥉 Βάθρο cell (three cells with spelled-out labels would overflow `max-w-game`). Tiered Podium Badges were **rejected** on 2026-08-06 and the reserved `leksokipos-first-place-*` id prefix is released unused. (Not: wins count)
