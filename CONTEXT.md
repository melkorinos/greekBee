...
**Rehearsal** — Replaying a Backup into a **local** PostgreSQL database and applying pending migrations against those real rows, to find out what a migration does before `db push` does it for real. Routine, repeatable, and never touches the hosted project. What it catches is **data-shape failure** — a `NOT NULL` meeting existing nulls, a unique index meeting existing duplicates — which is exactly the class an empty staging database passes. A pass means *the migration applied*, not *the suite is green*: the live-DB tests go through Supabase's HTTP API, which a local PostgreSQL install does not have. See ADR 0024 and `TICKET-13`. (Not: Restore, Backup, staging, dry run)
