...
- **Advisor baseline is noisy but expected** — `get_advisors security` returns a set of lints that are BY DESIGN: permissive anon-`INSERT` (and, where the app upserts/toggles, `UPDATE`/`ALL`) policies — public write is intentional on `game_state`, `player_achievements`, `player_pangrams`, `nominations`, `nomination_votes`, community tables — plus RLS-enabled-no-policy on the server-only tables (`identity_audit`, and `transfer_codes` since migration `20260716120000`) and auth leaked-password-protection off. Since migration `20260716120100` the three worst `rls_policy_always_true` ALL-grants are gone (per-command policies now). **Don't treat the remainder as regressions.** `get_advisors performance` is clean.
