...
- **Write volume is the real axis, and it is deliberately unaddressed.** `game_state` shows ~29,000 updates against 83 live rows (350:1) and `game_scores` ~31,500 against 536, because Leksokipos writes twice per word found and resends the whole `foundWords` array each time. **This is known and accepted** — see `TICKET-12`, which is gated on passing ~500 daily players. A session that rediscovers these ratios should read that ticket before proposing work.
