...
- **`npx vercel metrics <metric>` reads the whole Observability surface — CLI 58.11+ only (added after 58.9.4; the old "dashboard-only" note here was version drift, corrected 2026-08-13).** `vercel metrics schema` lists all 97 metrics. The ones that matter here: `vercel.function_invocation.function_cpu_time_ms` (**Fluid Active CPU**, in ms), `.function_duration_gbhr` (provisioned memory), `.count` (invocations), `vercel.request.fdt_out_bytes` (fast data transfer). Flags: `-a sum|avg|p75`, `--since 7d|30d`, `--granularity 1d`, `--group-by route`, `--json`, `--prod`. Needs `.vercel/project.json` (see linking recipe above) or `-p greek-bee`.
- **Billing period + prices: `npx vercel api "/v2/teams/<teamId>"`** (Git Bash mangles the leading slash — prefix `MSYS_NO_PATHCONV=1`). `billing.period` gives the cycle bounds; `billing.invoiceItems` gives unit prices **per 100 units** (`fluidCpuDuration` 12.8 = $0.128/CPU-hour, `fluidDuration` 1.06 = $0.0106/GB-hr, `fastDataTransfer` 15 = $0.15/GB with 1 TB included). The plan is Pro **`planIteration: "plus"`** — no per-metric free allotment; a single `includedAllocationUsd` of **$20/month** covers usage-based charges. `/v1/usage` rejects every timerange tried; `vercel contract` and `vercel alerts` both return empty.
...
- **13 tables, all RLS-enabled** (`public.` schema), **re-verified live 2026-08-15**: `game_scores` (536 rows / 392 kB), `game_state` (83 / 264 kB — holds **only `leksokipos`**), `nomination_votes` (219), `nominations` (190), `player_milestones` (93), `player_achievements` (120), `player_profiles` (47), `identity_audit` (8), `transfer_codes` (0), and 4 `community_*_puzzles` (1 row total). **`player_pangrams` and `player_words` NO LONGER EXIST** — both were absorbed into **`player_milestones`** (kinds `pangram`/`word`/`top_rank`/`tzimani`) by ADR 0013 / migration `20260807120000`. Don't look for them, and don't trust an older count of 14.
- **Size baseline:** `pg_database_size` = **13 MB** against the free tier's 500 MB, only ~1.5 MB of it these tables. **The billed figure is roughly double the SQL one** (the usage dashboard read 27.37 MB on 2026-08-12 for the same DB) — take size from the dashboard, use SQL for row counts. Growth ≈ **1.2 kB per active player per day**. Storage is not a constraint here; don't propose pruning without re-measuring.
- **Write volume is the real axis, and it is deliberately unaddressed — do not propose fixing it.** `game_state` shows ~29,700 updates against 85 live rows (350:1) and `game_scores` ~32,300 against 556, because Leksokipos writes **twice per word found** (state sync in `useGameStateSync` → `pushFoundWords`, which resends the whole `foundWords` array; plus one score POST per accepted word) — the ratios are one game's traffic, `game_state` holds only `leksokipos`. **Known and accepted.** Re-measured 2026-08-17: ~1,000 writes/day total, **3–9 daily players against a ~500 threshold**, so the ticket that specified the fix (debounce the state push, flush on `visibilitychange`/`pagehide`) was withdrawn rather than built. Revisit only when daily players cross ~500 or Vercel invocations become a visible bill line; the prize is ~5–10× efficiency nobody is paying for, and the risk is a dropped flush losing a player's words. Never debounce the score lane (it drives the live leaderboard) and never send delta words (ADR 0003's server-wins restore needs the full array).
