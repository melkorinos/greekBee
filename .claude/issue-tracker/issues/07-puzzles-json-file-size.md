...
2. Update `getPuzzleForDate` (and related loaders in `src/data/spelling-bee/index.ts`) to call `computeValidWords(centerLetter, outerLetters, allWords)` at request time, same as `buildCustomPuzzle`.
