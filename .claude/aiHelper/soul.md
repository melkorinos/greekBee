...
3. **Reflections** — **delete** resolved tensions from `reflections.md` (git history is the archive — there is no archive section any more); record newly discovered ones. **Grep first: if the lesson is already in the file, add your instance to that entry rather than opening a new section** (the rule `coverageMap.md` has for tests). **Hard cap: 120 lines** — the same cap as `memory.md` and `log.md`, because all three load every session and a file that grows without bound spends the budget before any work starts. When an entry outgrows a few lines, promote it to an ADR and leave a pointer (ADR 0026 is the worked example).
