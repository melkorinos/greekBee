...
### 🟡 Topothesies — DONE, awaiting operator play-through (s117)

Data + UI shipped; the only thing between here and go-live is the manual browser play-through the handoff reserves for the operator, then flipping `topothesies.wip:false`. **What the operator should sanity-check** (curation is theirs per ADR 0018 step 7): (1) **silhouette recognizability** — a `preview.html` gallery of all 101 shapes is in the session scratchpad; the 25% simplification keeps small islands as ~10–30-vertex shapes, but a few tiny ones (anafi, antiparos) are still crude; (2) the **Athens-4-sectors→one `athina` merge** (a v1 call not in the signoff — E/W Attica + Piraeus stay separate); (3) capital names/coords for the ~101 entries (auto-sourced from Wikidata muni seats, hand-named — some island capitals like «Χώρα X» were qualified to stay unambiguous in autocomplete). Nothing is half-built; the muni→RU spatial-match approach worked cleanly (only Thessaloniki/Sparta needed a hand fix). **s118 UX corrections applied** (capital stage has no distance hint; give-up button; on-screen decorative emoji stripped) — one thing to NOT "fix" later: the **share card keeps its Worldle emoji grid on purpose** (that grid is what renders when players paste results into messaging/social apps); only the on-screen play surface was de-emoji'd.
