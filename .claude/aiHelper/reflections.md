...
- **Pool reachability + blur tuning.** 150–200 icon-only *and* recognizable brands is ambitious; defunct/nostalgia marks are what make it reachable. If it stalls, relax "recognizable" before "icon-only" (relaxing icon-only breaks the game). Blur difficulty is a `LOGOPAIGNIO.BLUR_STEP_RADII_PX` knob — tune without code.
