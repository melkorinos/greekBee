...
6. Prod-readiness state: play-through done by user (pre-rework), gates green pre-rework (1561); **rework needs fresh local verify by user + gates** before commit → dev→main. Leksindeseis `wip: true` is longstanding (since registry creation), not a regression — user decision pending (user has since edited games.ts directly).
7. **Continuous score posting, BOTH games** (user: "people won't finish"): boards now call `submit(getRoundScore/getTotalScore)` on every live state change — `useScoreSubmission`'s strictly-increasing guard dedups; restores never post (userActedRef); leaderboard still auto-opens at finish. Leksoplegma mid-round posts lack `is_perfect` (latch fires at finish).
8. **Leksodromia Second Chance** (grilled: second-pass + resumed clock): first «Επόμενο» REQUEUES the word as a new step (`state.retries[step] = {origIndex, baseElapsedMs, baseHints}` — no result recorded); clock+hints RESUME on the revisit (anti peek-skip-solve-offline exploit); second skip is final (0 pts). New selectors `getTotalSteps/isSecondChance/getCurrentAnswer/getCurrentScramble/getRetryBaseElapsedMs`; hook resets clock to retry base + persists `retries` (old snapshots default `{}`); board: «Λέξη x/totalSteps», 🔁 «2η ευκαιρία» badge, armed-skip copy differs per pass («ξανά στο τέλος» vs «0 πόντοι»); HowToPlay + home blurb updated.
9. **Greeklish renames** (UI strings only, per locked decision): Λεξοδρομία→Leksodromia, Λεξόπλεγμα→Leksoplegma in registry label/title, both page h1s, home rulesTitles; CONTEXT.md glossary terms normalized to greeklish. Routes/SliceIds/types untouched.
