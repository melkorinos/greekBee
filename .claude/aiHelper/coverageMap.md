...
| `IdentityHeader` / `LifetimeStatsStrip` / `NameEditor` / `TrophyCase` / `WelcomeBackBanner` / `WordsByLengthCard` (profile) | The six Profile Page components. **`TrophyCase` post-TICKET-02:** five tiered tiles, four live values off ONE stats response mapped per badge (a "keeps each badge's live value on its own tile" row locks the wiring), pangram chips at 25/60/150, day-count chips + progress for Στην Κορυφή / Τζιμάνι, medals, and the base-id-never-tier-id picker |
