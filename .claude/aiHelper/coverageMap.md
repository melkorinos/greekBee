...
| `registryCoverage.test.tsx` | GAME_REGISTRY → surface contracts, three seams: drawer derives from the registry (probe Game injected via a module mock), every Game has a `[data-game]` accent rule in globals.css (+ no orphan rules), every Game has a leaderboard config or sits in `NO_LEADERBOARD_IDS`. Behavioural drawer tests stay in `Shell.test.tsx` — this file owns only the divergence guards |
