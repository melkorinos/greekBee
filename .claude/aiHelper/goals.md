...
2. **The five launch tickets** (`.claude/tracker/tickets/`, all from question 1): **`TICKET-06`** hide the three `wip:true` Games from picker, drawer and Offline Mode behind a new `hidden` registry state (ADR owed — `games.ts` documents its state design deliberately); **`TICKET-07`** a Greek privacy page; **`TICKET-08`** error monitoring, decision-shaped (compare, choose, install *or* decline in writing); **`TICKET-09`** operational headroom, operator work; **`TICKET-10`** an Open Graph card and favicon. Two ordering constraints: **08 before 07**, and **06 before the play-through**.
...
7. **Leksindeseis** — `wip: true`, **hidden at launch** (`TICKET-06`). Its static-fallback pool is **one placeholder puzzle**, measured 2026-08-11, with no reminder system (see `reflections.md`). Parked while the Game is hidden; live again the moment a flip is considered.
