...
2. **The launch tickets** (`.claude/tracker/tickets/`, all from question 1): **`TICKET-06` SHIPPED 2026-08-12** — the three Games are hidden behind a new `hidden` registry state, orthogonal to `wip`, routes still live (ADR 0022); **`TICKET-07` SHIPPED 2026-08-12** — `/privacy` in Greek, one drawer link, and the Feedback payload cut to two fields (FormSubmit was already a third-party processor); **`TICKET-08`** error monitoring, **decided** (no third-party SDK, Vercel's built-in observability) — the ADR, a scheduled manual check and a preview-verified thrown error are what is left; **`TICKET-09`** operational headroom, operator work; **`TICKET-10`** an Open Graph card and favicon. **No ordering constraints remain**: 06 shipped before the play-through, and the 08-before-07 constraint was withdrawn once the SDK question was settled.
