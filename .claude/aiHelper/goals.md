...
**The active epic is public launch readiness**, charted as a wayfinder map at `.claude/issue-tracker/tickets/00-MAP-public-launch-readiness.md` (created 2026-07-31). The destination is *"nothing is blocking a launch decision"* — the date itself stays the operator's. Read the map first; it is the authoritative list of what is open and what has already been decided.

1. **Work the map's frontier.** Three tickets are open and unclaimed: **01** the launch checklist (what "launch-ready" actually requires — everything else is sized against it), **02** the UI scope verdict (a few tweaks or the full redesign epic), and **09** the launch sequencing (blocked by both). Tickets 03–08 are resolved and folded into the map's "Decisions so far".
2. **UI redesign (scope undecided — that is ticket 02).** The seams are ready (sessions 102–104; ADR 0008/0009 extensions): the whole redesign surface is `globals.css` + `recipes.ts` + `Modal.tsx` + `GamePageShell`/`GameHeader` + `GameLeaderboardModal`. Open decisions to make *during* the redesign: full-bleed vs padded game headers (Leksokipos keeps a bespoke full-bleed wrapper until then), real accent colours for stavrolekso/leksikastirio (current sky/indigo rows are placeholders), and whether to tokenise `FeedbackBanner` + drop its `theme` prop (visible change to Leksiarxeio's banner — see ADR 0008 exceptions).
3. **Bug/task issues** — `.claude/issue-tracker/issues/` holds exactly one open item: **02**, no disaster-recovery backups. Everything else was resolved and deleted, per the standing rule. Larger open *questions* live on the map above, not here.
4. **Untracked work that still needs doing** — the badge catalog rebuild (`.claude/handoffs/badgeIdeas.md` + `badgeVisualSystem.md`), Πόσο κάνει; content sourcing (no ticket survives — see `reflections.md`), and Λογοπαίγνιο's content pool (deliberately off the launch map, `.claude/handoffs/logopaignio-content-pool.md`).
5. **E2E coverage** — Playwright is wired (`npm run test:e2e`); grow happy-path coverage per game. Whether the current suite is a launch gate is part of ticket 01.
