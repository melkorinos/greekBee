...
3. **The tracker** (`.claude/tracker/`) — `tickets/` is **empty**: `TICKET-01` (`player_milestones`) shipped 2026-08-07 and `TICKET-02` (catalog rebuild + launch reset) the same day, both files deleted per the standing rule. `issues/` holds two deferred problems: `ISSUE-01` (no disaster-recovery backups) and `ISSUE-02` (`rlsInvariantsLiveDb` flakes under full-suite load). **Owed and not a ticket: the after-hours deploy window** — `npx supabase db push` then the Vercel deploy, back to back, shipping both tickets at once; and separately, on release day, `supabase/scripts/launch-reset.sql`. Open *questions* live in the handoff above, not here.
4. **Untracked work that still needs doing** — Πόσο κάνει; content sourcing (nothing survives — see `reflections.md`), badge art (now `TICKET-03`, decoupled on purpose), and Λογοπαίγνιο's content pool (deliberately out of scope, `.claude/handoffs/logopaignio-content-pool.md`).
