...
2. **The launch tickets.** All of question 1's tickets are in `.claude/tracker/tickets/`, and **what is still open is read from that folder, never from here** — this file kept a per-ticket status table until it went stale twice. What belongs in *goals* is the shape: five tickets came out of question 1, the shipped ones are deleted per the standing rule, and **no ordering constraints remain between any of them** (the 08-before-07 one was withdrawn in s149, the logo→10 one dissolved in s154 when the logo folded *into* `TICKET-10`).
3. **The standing non-launch item is Sound Cues.** `TICKET-04` built the whole primitive (ADR 0021); s154 split `TICKET-05` and shipped Part A, a `FEATURE_FLAGS.soundCues` gate hiding the 🔊 button — so sourcing the three MP3s is **post-launch, optional, and blocks nothing**. **Owed and not a ticket:** the play-through of `dev` on preview (Leksodromia, Leksoplegma, the sessions 102–104 visual shifts, the badge art eye-check in both themes) and the release-day runbook in the handoff.
4. **Operator-driven work that is deliberately not tracked in the handoff** — the **UI redesign**, run in separate sessions. The **platform logo** stopped being a separate thread in s154: the icon picked from the candidates page *is* the mark, so it lives inside `TICKET-10`. Πόσο κάνει; and Λογοπαίγνιο content are **no longer pending** — both Games are `hidden` (ADR 0022), so their content is out of scope until unhiding is considered.
