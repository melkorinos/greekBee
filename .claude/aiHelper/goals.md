...
3. **The tracker** (`.claude/tracker/`) — `tickets/` holds **`TICKET-05`** alone: source the three Sound Cue MP3s, **operator work** (nobody can eye-check a sound), spec'd by **ADR 0021**. `TICKET-04` built the whole primitive on 2026-08-11, so the 🔊 toggle now renders on every page **playing silence** — the feature is pre-launch and explicitly non-blocking, but it is **merged and must not deploy** until the files land. Numbers are never reused: `TICKET-01`/`02` shipped 2026-08-07, `TICKET-03` 2026-08-10 and `TICKET-04` 2026-08-11, all four files deleted per the standing rule. `issues/` holds only `ISSUE-01` (no disaster-recovery backups) — **`ISSUE-02` is referenced here and in `memory.md` but its file does not exist**, and s144 reported the `rlsInvariantsLiveDb` failures it described are now gone; resolve or re-file it. **Owed and not a ticket: the after-hours deploy window** — `npx supabase db push` then the Vercel deploy, back to back, shipping both tickets at once; and separately, on release day, `supabase/scripts/launch-reset.sql`. Open *questions* live in the handoff above, not here.
