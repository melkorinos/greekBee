...
2. **The launch tickets** (`.claude/tracker/tickets/`, all from question 1): **`TICKET-06` SHIPPED 2026-08-12** — the three Games are hidden behind a new `hidden` registry state, orthogonal to `wip`, routes still live (ADR 0022); **`TICKET-07` SHIPPED 2026-08-12** — `/privacy` in Greek, one drawer link, and the Feedback payload cut to two fields (FormSubmit was already a third-party processor); **`TICKET-08` SHIPPED 2026-08-12** — ADR 0023 records the no-SDK decision, the daily `vercel logs` check that replaces alerting, and a real induced error retrieved end-to-end; **`TICKET-09` CLOSED 2026-08-13** — the operator read the last two numbers, accepted the row-count-alert substitution, and the whole headroom picture now lives in the `reflections.md` rate-limiting entry; **`TICKET-10` IN-PROGRESS** — its metadata half shipped 2026-08-13 (`PLATFORM_DESCRIPTION` filters `hidden` behind a guard test, plus `metadataBase`/`openGraph`/`twitter`/`PLATFORM_ORIGIN`); the image half is **blocked on one operator pick** from `html/share-card-candidates.html`. **No ordering constraints remain**: 06 shipped before the play-through, and the 08-before-07 constraint was withdrawn once the SDK question was settled.
3. **The tracker's standing items.** `tickets/` also holds **`TICKET-05`** — source the three Sound Cue MP3s, **operator work** (nobody can eye-check a sound), spec'd by **ADR 0021**. `TICKET-04` built the whole primitive on 2026-08-11, so the 🔊 toggle renders on every page **playing silence**: the feature is explicitly non-blocking, but it is **merged and must not deploy** until the files land. Numbers are never reused: `01`/`02` shipped 2026-08-07, `03` 2026-08-10, `04` 2026-08-11, all deleted per the standing rule. `issues/` holds `ISSUE-01` (no disaster-recovery backups), `ISSUE-03` (thin E2E coverage, deferred 2026-08-11) and `ISSUE-04` (the intermittent `rlsInvariantsLiveDb` `game_state` DELETE flake, re-filed 2026-08-14 — it had been cited as `ISSUE-02` with no file on disk; `02` stays spent). **Owed and not a ticket:** the play-through of `dev` on preview (Leksodromia, Leksoplegma, the sessions 102–104 visual shifts, and the badge art eye-check in both themes), and the release-day runbook in the handoff.
4. **Operator-driven work that is deliberately not tracked in the handoff** — the **UI redesign** (item 2's surface, run in separate sessions) and the **platform logo**. The logo is the only genuinely untracked item on the launch path, and **`TICKET-10` depends on it**: the Open Graph card and the favicon both need a mark, so either it lands first or `TICKET-10` ships a placeholder and is revisited. Πόσο κάνει; and Λογοπαίγνιο content are **no longer pending** — both Games are `hidden` (ADR 0022), so their content is out of scope until unhiding is considered.
