...
describe("buildInitialState", () => {
  ...
  it("starts at Beginner rank", () => {
    const s = freshState();
    expect(s.currentRank).toBe("Beginner");
  });
...
describe("SUBMIT_WORD — valid word", () => {
  ...
  it("advances the rank when threshold is crossed", () => {
    ...
    expect(s.currentRank).not.toBe("Beginner");
  });
...
describe("NEW_GAME", () => {
  it("resets all progress", () => {
    ...
    const newPuzzle: Puzzle = { ...puzzle, id: "new-puzzle" };
...
describe("RESTORE_STATE", () => {
  it("merges saved fields into current state", () => {
    ...
    expect(s.currentRank).toBe("Moving Up");
  });
