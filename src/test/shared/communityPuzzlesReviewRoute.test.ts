...
// Tests for PATCH /api/community-puzzles/leksindeseis/[id]/review — the auth
// and action logic every review route shares, exercised through the one Game
// that still schedules a Community Puzzle (ADR 0027 removed the other two).
...
describe("PATCH review — auth", () => {
  it("returns 401 with no X-Admin-Secret header", async () => {
    const req = makePatchReq("leksindeseis", "1", { action: "approve" });
    const res = await PATCH_LEKSINDESEIS(req, params("1"));
    expect(res.status).toBe(401);
  });

  it("returns 401 with wrong secret", async () => {
    const req = makePatchReq("leksindeseis", "1", { action: "approve" }, "wrong");
    const res = await PATCH_LEKSINDESEIS(req, params("1"));
    expect(res.status).toBe(401);
  });
  ...
  it("returns 400 for invalid JSON", async () => {
    const req = new NextRequest(
      "http://localhost/api/community-puzzles/leksindeseis/1/review",
  ...
  it("returns 400 for unknown action", async () => {
    const req = makePatchReq("leksindeseis", "1", { action: "suspend" }, CORRECT_SECRET);
    const res = await PATCH_LEKSINDESEIS(req, params("1"));
    expect(res.status).toBe(400);
  });
});
