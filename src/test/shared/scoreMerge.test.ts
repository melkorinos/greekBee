// scoreMerge.test.ts — planScoreMerge, the pure fold over game_scores rows.
//
// When a returning player signs in on a new device (ADR 0012),
...
// Deleting the loser row, rather than mutating the winner's score, keeps the
// surviving row exactly as its Game posted it.
//
// `mergeLengthScore` was covered here too until ADR 0027 removed Λεξιαρχείο's
// scoring and the per-length fold with it.
