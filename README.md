...
| 3 | **Spelling Bee max-score cap is a blunt instrument** | `maxScore()` hard-caps at 500 pts to prevent leaderboard inflation on large-word-list puzzles. A better approach: use a word-count percentile (e.g. target the 70th-percentile word by frequency) so all puzzles feel equally challenging regardless of raw dictionary coverage. |
| 4 | **Wordle answer pool quality** | `answers-5.json` was generated by frequency filter but still includes obscure / archaic Greek words. Should be manually curated or filtered against a high-frequency lemma list so daily Wordle answers are always common, recognisable words. |
