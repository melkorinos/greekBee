...
- **Open questions are neither.** They live in `.claude/handoffs/`, and **none is open as of 2026-08-20** — the last, sequencing the launch run, was answered into `docs/launch-runbook.md`, which owns the order of work to the go/no-go and release day itself. Read that folder before assuming an open question is untracked. Resolving one produces an ADR or `CONTEXT.md` entry plus, usually, tickets. Wayfinder is retired; there is no map.
