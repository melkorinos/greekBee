...
- **Styling uses semantic tokens** (`bg-surface`, `text-muted`, `border-border`, `bg-inverted`, `text-correct`, `bg-warning-surface`…), never literal neutral palette classes (`stone-`/`zinc-`/`gray-`…) or hand-written `dark:` pairs (ADR 0008). Composite recipes live in `src/styles/recipes.ts` (cross-game) / `src/components/leksokipos/styles.ts` (Leksokipos-only) — reuse them, don't re-roll button/input/label/tooltip/card strings; a recipe owns colour/typography/radius, the call site owns layout. Game pages frame through `GamePageShell` + `GameHeader` (`src/components/shared/`), and the content column is `max-w-game` (`--container-game`) — never a literal `max-w-sm` (`noLiteralColumnWidth.test.ts` enforces). Intentional non-token exceptions are enumerated in ADR 0008 and guarded by `noRawPaletteClasses.test.ts`; add a new one only by editing that allowlist + the ADR.
