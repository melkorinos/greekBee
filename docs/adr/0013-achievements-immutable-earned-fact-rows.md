...
**DB.** The `game_scores.is_perfect` column was **kept** at the time (no migration) — data stayed, nothing read it, and an optional drop could ride a future migration. **It rode one: dropped 2026-08-20** in `20260820120000_drop_two_community_queues_and_dead_score_columns.sql`, alongside the now-dead `data` jsonb (ADR 0027 §5). Measured before the drop: 0 rows true, 583 false, and the only `is_perfect` references anywhere in `src/` were the three generated ones in `database.types.ts`. The column no longer exists; nothing here is owed to it.
