...
- **Admin Restore** is a break-glass SQL recipe (`docs/admin-restore.md`): email → `auth_user_id` → `device_uuid` → insert a TransferCode the player claims normally. Residual gap accepted: if a player disconnects Google *and* loses their device, the email hop is gone — mitigated by an append-only `identity_audit` log of unlink events, written when Google is disconnected, so the recipe has a last-known mapping.
