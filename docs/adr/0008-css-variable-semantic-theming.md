...
- **Semantic token set:** `surface`, `surface-raised`, `foreground`, `muted`, `border`, `brand`, `accent`, `danger`, `success`, plus a feedback group `correct` / `present` / `absent` / `misplaced`. The platform references only these — no literal palette classes (`yellow-400`, `stone-600`) in components.
  - **`success`** (green-600 light / green-500 dark) is the counterpart to `danger` for *action* semantics — the approve/confirm button fill (`btnApprove`), paired with `danger` for reject/destructive (`btnReject`). Added when the Leksikastirio review cards + NominationCard were found hand-rolling `bg-green-600 …` / `bg-red-500 …` in ~10 places. Distinct from the `correct` *feedback* token (a tile state): retuning a Wordle tile must not move an admin button. Error *text* (`text-danger`) also stopped using literal `text-red-500/400` at the same time.
...
- **Composite recipes** (`btnPrimary`, inputs, leaderboard rows) move from `src/components/leksokipos/styles.ts` to a platform-scoped `src/styles/` home — they are not game-specific. (Refined by [ADR 0009](./0009-per-game-accent-and-chrome-consolidation.md): recipes that turned out to be Leksokipos-only — pangram/word-chip/score-bar/feedback — moved *back* to `src/components/leksokipos/styles.ts`; `src/styles/recipes.ts` keeps only genuinely cross-game recipes.)
  - **`keyStruck`** (2026-08-29) — the eliminated key's diagonal strike, in every game that draws a Wordle-style keyboard. The decision it records is that **a state a player must act on is worth a shape, not only a colour**: dark mode had been painting an eliminated key *lighter* than a key still in play, a second pair of grey tokens (`key-idle` / `key-absent`) was tried and reverted, and a strike says it in one glance without asking anyone to rank two greys — or to see hue at all. It draws in `currentColor`, so it needs no token and no `dark:` pair, and the four feedback colours keep meaning only what they already mean. Guarded in `src/test/shared/keyOutOfPlay.test.tsx`, which also pins that Enter and Delete keep exactly the untouched-letter fill.
...
**Enforcement:** two build-failing guards.
- `noRawPaletteClasses.test.ts` — the neutral-chrome migration is effectively complete; it fails if any component `.tsx` introduces a literal `stone/zinc/gray/slate/neutral` class outside the allowlisted exception files above. New exceptions require editing that allowlist *and* this section.
- `noRawActionButtonColors.test.ts` — narrower: forbids the solid action-button fills `bg-{green,red}-{500,600,700}` anywhere in shipped `.tsx`, so approve/reject buttons keep using the `btnApprove`/`btnReject` recipes (`success`/`danger` tokens). Fixed-meaning green/red *tints & scales* (difficulty tiers, the Stavrolekso solved cell, status-banner tints, text-only accents) use other shades and are deliberately not matched — hence no allowlist. Accent colours in general (amber/sky/blue/purple/yellow) remain ungoverned by design.
