...
- **Semantic token set:** `surface`, `surface-raised`, `foreground`, `muted`, `border`, `brand`, `accent`, `danger`, `success`, plus a feedback group `correct` / `present` / `absent` / `misplaced`. The platform references only these — no literal palette classes (`yellow-400`, `stone-600`) in components.
  ...
  - **`key-idle` / `key-absent`** — the on-screen keyboard's two neutral fills, split out of `border` and `absent` (2026-08-26). Both of those are pinned elsewhere — `absent` is a feedback colour the guess tiles hold identical in both themes, `border` is page chrome — which left dark mode painting an eliminated key *lighter* than a key still in play. The keyboard keeps one rule in both themes now: eliminated recedes, still-in-play stands out. Light values compile to the previous classes; the dark ordering is guarded in `src/test/leksiarxeio/theme.test.tsx`.
