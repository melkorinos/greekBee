...
- **Value correctness is still not enforced, and this is the honest limit of the change.** `status` generates as `string`, not a union, because Postgres CHECK constraints do not survive into TypeScript — only real PG enums do, and this DB has none (`Enums: {}`). So `status: "approved"` and `status: "utter_garbage"` both compile. The old hand-written union was not stale so much as **aspirational**: it encoded a constraint the database never had. The community_* tables have no CHECK on `status` at all, which is why `"approved"` worked in production while contradicting the type. Tracked in `.claude/handoffs/status-check-constraint-handoff.md`.
